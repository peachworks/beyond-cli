    ...
    login(this.config)
      ...
      .then(triggers => {
        ...
        return Bluebird.each(triggers, trigger => {
          let filePath = path.join(baseDir, trigger.object_id.api_name, 'triggers', trigger.file_id.name)
      ...
      .then(() => {
        return this.api.getObservers()
      })
      .then(observers => {
        let baseDir = this.config.env

        // for each entry create file with contents overwriting existing ones
        return Bluebird.each(observers, observer => {
          let filePath = path.join(baseDir, observer.object_id.api_name, 'observers', observer.file_id.name)
          console.log(`Pulling observer`, observer.id, filePath, observer.file_id.content.length)

          fs.mkdirSync(path.dirname(filePath), {
            recursive: true
          })
          fs.writeFileSync(filePath, observer.file_id.content)
        })
      })
      .catch(err => {
        console.error(err)
