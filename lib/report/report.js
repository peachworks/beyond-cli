...
const constants = require('../../lib/const'),
  fs = require('fs'),
  JsReport = require('./js-report'),
  PaqlReport = require('./paql-report'),
  path = require('path'),
...
class Report {
  ...
  print(response) {
    if (response.error) {
      ...
      if (response.results.length === 0) {
        ...
        if (this.config.originalParams.export) {
          this.exportToCsv(response, this.config.originalParams.export);
        } else {
  ...
  parseDisplayParameters(reportFileName) {
    ...
    if (this.config.originalParams.params && !this.config.originalParams.params.endsWith('.json')) {
      params = this.parseRawParameters(this.config.originalParams.params);
    } else {
  ...
  getParamsFileName(reportFileName) {
    if (this.config.originalParams.params) {
      if (path.extname(this.config.originalParams.params) !== '.json') {
        throw 'Only *.json files are supported as params or raw JSON or list of params separated by comma';
      }

      return path.join(process.cwd(), this.config.originalParams.params);
    }
