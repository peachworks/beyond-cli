...
function api(config) {
  ...
  function getTriggers() {
    return apiRequestPromise({
      ...
      .then(result => {
        ...
        console.log(`TRGS`, triggers.length)
  ...
  function getObservers() {
    return apiRequestPromise({
      ...
      // decorate triggers with file data
      .then(result => {
        let observers = result.results
        console.log(`OBSERVERS`, observers.length)
        let fileIds = _.map(observers, 'file_id')
        return apiRequestPromise({
          uri: '/v1/files',
          qs: {
            find: JSON.stringify({
          ...
          .then(result => {
            let files = result.results
            let filesMap = _.keyBy(files, 'id')
            return _.map(observers, observer => {
              observer.file_id = filesMap[observer.file_id]
              return observer
            })
          })
      })
      // decorate triggers with object data
      .then(observers => {
        let objectIds = _.map(observers, 'object_id')
        return apiRequestPromise({
          uri: '/v1/objects',
          qs: {
            find: JSON.stringify({
          ...
          .then(result => {
            ...
            let objectsMap = _.keyBy(objects, 'id')
            return _.map(observers, observer => {
              observer.object_id = objectsMap[observer.object_id]
              return observer
            })
          })
      })
